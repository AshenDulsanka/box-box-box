#!/usr/bin/env python3
"""
Machine Learning approach: Use scipy optimization to find the exact formula.
"""
...
def simulate_driver_formula1(strategy, total_laps, base_lap_time, pit_lane_time, track_temp,
                              offsets, thresholds, rates, tb, temp_coeff):
    """Formula 1: Cumulative degradation after threshold"""
    ...
    for lap in range(1, total_laps + 1):
        ...
        temp_factor = 1 + max(0, track_temp - tb) * temp_coeff
...
def test_params(params, test_cases):
    ...
    for tc_data in test_cases:
        ...
        for strat in strategies.values():
            time = simulate_driver_formula1(strat, race_config['total_laps'], race_config['base_lap_time'],
...
def main():
    ...
    # Bounds: [oM, oH, tS, tM, tH, rS, rM, rH, tb, tc]
    bounds = [
        (0.1, 5.0),    # oM
        (0.2, 8.0),    # oH
        (1, 50),       # tS
    ...
    result = differential_evolution(
        lambda p: test_params(p, test_cases),
        bounds,
        maxiter=500,
        popsize=30,
        tol=0.001,
